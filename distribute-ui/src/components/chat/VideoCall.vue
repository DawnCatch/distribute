<template>
    <div class="video_call_box" ref="videoCallBox" :style="style" v-show="false">
        <div>
...
onMounted(() => {
    mitt.on("rtc:request", (userId) => {
        ...
        start().then((stream: MediaStream | undefined) => {
            localVideo.value && remoteVideo.value && stream && createPeer(userId as number, stream, localVideo.value, remoteVideo.value).then((connect) => {
                peer = connect
            })
        })
    })
    ...
    mitt.on("rtc:callback", (e) => {
        ...
        if (data.status) {
            wait = true
            start().then((stream: MediaStream | undefined) => {
                createAnswer(data.userId, stream!!, data.session, localVideo.value!!, remoteVideo.value!!).then((connect) => {
                    peer = connect
                })
            })
        } else {
