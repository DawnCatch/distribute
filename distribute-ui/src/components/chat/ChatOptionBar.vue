...
import { useElementSize, useTextareaAutosize, useFocusWithin, watchDebounced, useDraggable } from '@vueuse/core';
...
function handleKeyCode(event: KeyboardEvent) {
    if (event.key === "Enter" && event.ctrlKey && input.value !== "") {
        ...
        input.value = input.value.substring(0, selectionStart) + "    " + input.value.substring(selectionStart)
        nextTick(() => {
